from app.application.api.depends import pricing_config_service_deps
...
async def create_pricing_config(request: PricingConfigCreate, pricing_config_service: pricing_config_service_deps):
...
async def get_pricing_config(reo_id: int, pricing_config_service: pricing_config_service_deps):
    config = await pricing_config_service.get_pricing_config(reo_id=reo_id)
    return config


@router.get("/", response_model=list[PricingConfigResponse])
async def get_all_pricing_configs(pricing_config_service: pricing_config_service_deps):
...
async def update_pricing_config(
    id: int, request: PricingConfigUpdate, pricing_config_service: pricing_config_service_deps
):
...
async def delete_pricing_config(id: int, pricing_config_service: pricing_config_service_deps):
