...
class DistributionConfigsService:
    ...
    async def get(self, config_id: int) -> DistributionConfigResponse:
        distribution_config = await self.repository.get(config_id)
        if not distribution_config:
            raise ObjectNotFound(model_name="DistributionConfig", id_=config_id)
        return DistributionConfigResponse.model_validate(distribution_config)

    async def update(self, config_id: int, data: DistributionConfigUpdate) -> DistributionConfigResponse:
        distribution_config = await self.repository.get(config_id)
        if not distribution_config:
            raise ObjectNotFound(model_name="DistributionConfig", id_=config_id)
    ...
    async def get_all(self) -> list[DistributionConfigResponse]:
